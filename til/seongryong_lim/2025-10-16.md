Python에서 redis를 사용하는 것은 매우 간단하다!! 마참내!
...
1. redis와 연결을 한다.
    ...
    r = redis.Redis(
        ...
        decode_responses=<Boolean, default=False> # 디코딩된 문자열 수신
    )
    ```
...
| MSET | 여러 키에 대한 값을 한 번에 저장한다 | MSET mine milk yours coffee |
| MGET | 여러 키에 대한 값을 한 번에 얻어온다. | MGET mine yours |
...
| ZRANK | 특정 값의 위치(순위) 반환 (오름차순 정렬된 기준) | ZRANK myrank apple |
| ZREVRANK | 특정 값의 위치(순위) 반환 (내림차순 정렬된 기준) | ZERVRANK myrank apple |
...
![image.png](../../asset/임성룡/redis-bitmap.png)
...
| BITOP
(BIT Operation) | 비트맵들간의 비트 연산을 수행하고 결과를 비트맵에 저장 | BITOP AND | XOR | OR result today yesterday |
...
| PFMERGE | 다수의 HyperLogLog를 병합 | PFMERGER result visit1 visit2 |
