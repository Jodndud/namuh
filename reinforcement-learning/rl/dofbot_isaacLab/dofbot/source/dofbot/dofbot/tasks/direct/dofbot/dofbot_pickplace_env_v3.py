...
class DofbotPickPlaceEnvV3(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        rlink2_pos = self.robot.data.body_pos_w[
            :, self._gripper_bodies_idx[0]
        ]  # rlink2
        llink2_pos = self.robot.data.body_pos_w[
            :, self._gripper_bodies_idx[1]
        ]  # llink2
        ...
        # Distances
        d_ee_obj = torch.linalg.norm(
            ee_pos_w - obj_pos_w, dim=-1
        )  # EE (rlink2) to object
        d_gripper_obj = torch.linalg.norm(
            gripper_center - obj_pos_w, dim=-1
        )  # Gripper center to object
        ...
        near_object = (
            d_gripper_obj < self.cfg.grasp_contact_threshold
        )  # 0.03 (양쪽 finger tip 중심 기준)
        ...
        if grasped.any():
            lift_progress = torch.clamp(
                obj_height / self.cfg.lift_height_threshold, max=1.0
            )
        ...
        if (grasped & lifted).any():
            ...
            # At goal bonus
            reward += (
                self.cfg.rew_place_success_bonus * (grasped & lifted & at_goal).float()
            )
    ...
    def _sample_xy(
        ...
        if self._placement_x_only:
            ...
            if front_only and min_radius > 0:
                ...
                y = torch.where(
                    y < min_radius,
                    min_radius + torch.rand_like(y) * (hi - min_radius),
                    y,
                )
            xy[:, 1] = y
        else:
